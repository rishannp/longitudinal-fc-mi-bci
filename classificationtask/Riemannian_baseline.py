# -*- coding: utf-8 -*-
"""
Riemannian baseline replicating the CSP experiment pipeline exactly.
...
5) SPD covariance matrices per trial, TangentSpace fit on TRAIN only
6) Standardise (train-only), PCA, GridSearchCV on training only, evaluate on test
...
def build_trials_for_subject(d):
    """Returns flat list of trials [n_times, n_ch] and y (0=L, 1=R). Variable length ok."""
...
# CRITICAL: TangentSpace fit on TRAIN only
# -----------------------------
def compute_spd_cov(trial_nch_nt):
    """trial: [n_times, n_ch] -> [n_ch, n_ch] SPD covariance."""
    x = trial_nch_nt  # already [n_times, n_ch]
    x = (x - x.mean(axis=0)) / (x.std(axis=0) + 1e-8)
    cov = np.cov(x.T) + 1e-10 * np.eye(x.shape[1])
    return cov


def extract_riemannian_features_temporal(trials, y, subj_id):
    """
    trials: list of [n_times, n_ch] arrays — variable length ok.
    Fits TangentSpace on TRAIN split only, transforms all trials.
    Returns X_feat [n_trials, n_features] aligned to same trial order.
    """
    split_idx = int(np.floor(0.2 * len(trials)))
    X_train_ep = trials[:split_idx]
    X_test_ep = trials[split_idx:]
    y_train = y[:split_idx]

    if len(np.unique(y_train)) < 2:
        print(f"[WARN] {subj_id}: cannot fit TangentSpace (train split has one class).")
        return None

    C_train = np.stack([compute_spd_cov(tr) for tr in X_train_ep], axis=0)
    C_test = np.stack([compute_spd_cov(tr) for tr in X_test_ep], axis=0)

    ts = TangentSpace(metric="riemann")
    F_train = ts.fit_transform(C_train, y_train)
    F_test = ts.transform(C_test)

    return np.vstack([F_train, F_test])
...
def evaluate_models_temporal(X, y, subj_id, feature_type="Riemannian"):
...
# -----------------------------
def compute_session_accuracy(X_riemann, y, n_sessions=4):
    """
    Train on first 20% (same split as classifier).
    Evaluate balanced accuracy on each equal third of the remaining 80%.
    Returns [train_block_acc, test_third1_acc, test_third2_acc, test_third3_acc].
    """
    n_trials = X_riemann.shape[0]
    split_idx = int(np.floor(0.2 * n_trials))

    scaler = StandardScaler()
    X_train = scaler.fit_transform(X_riemann[:split_idx])
    y_train = y[:split_idx]
    ...
    X_test_all = X_riemann[split_idx:]
...
# Visualisation
# -----------------------------
def plot_feature_space(X_riemann, y, subj_id):
    """One figure per subject. Session inferred by trial quartile. Session=colour, Class=marker."""
    ...
    scaler = StandardScaler()
    X_std = scaler.fit_transform(X_riemann)
    ...
    ax.set_title(f"Riemannian Tangent Space (PC1 vs PC2) — {subj_id}", fontsize=11, fontweight="bold")
    ax.set_xlabel("PC1", fontsize=10)
    ax.set_ylabel("PC2", fontsize=10)
    plt.tight_layout()

    fname = f"Riemannian_feature_space_{subj_id}.png"
    plt.savefig(fname, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {fname}")


def plot_global_accuracy(subject_session_acc, fname="Riemannian_accuracy_chart.png"):
    ...
    ax.set_title("Riemannian: Session-wise Balanced Accuracy (Train=S1 block, Test=S2–S4 blocks)",
...
for sid, d in data_by_subject.items():
    ...
    # Riemannian tangent space features — TangentSpace fit on TRAIN only
    X_riemann = extract_riemannian_features_temporal(trials, y, sid)
    if X_riemann is None:
        continue
    print(f"  Riemannian feature matrix: {X_riemann.shape}")

    # Classification: 20/80 temporal split, standardise+PCA on train only
    res = evaluate_models_temporal(X_riemann, y, sid, feature_type="Riemannian")
    all_results_riemann[sid] = res

    # Feature space plot
    plot_feature_space(X_riemann, y, sid)

    # Session-wise accuracy for global chart
    subject_session_acc[sid] = compute_session_accuracy(X_riemann, y)
...
# Save CSV
rows = []
for subj_id, model_dict in all_results_riemann.items():
    for model_name, metrics in model_dict.items():
        row = {"Subject": subj_id, "Model": model_name, "FeatureType": "Riemannian_TangentSpace"}
        row.update(metrics)
        rows.append(row)
pd.DataFrame(rows).to_csv("RIEMANNIAN_RESULTS_FULL.csv", index=False)
print("\nSaved CSV: RIEMANNIAN_RESULTS_FULL.csv")